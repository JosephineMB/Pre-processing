...
```{r }
`2b9c_28nov` <- read.csv2("~/Devellyn/traitement_data position/Firminy installation/Analyse de trajectoires/data/2b9c-28nov21.csv")

tagId <- unique(`2b9c_28nov`$tag)

data <- `2b9c_28nov` 
x <- intersect(which(data$date> "2021-11-28 15:00:00"), which(data$date< "2021-11-28 16:00:00"))
data<- data[x,]
...
# points(data$measuredX,data$measuredY,col=3, cex=0.1, pch=20)
  ...
  polyg.list
  points(corr.data$x,corr.data$y, pch=20, col=2, cex=0.1)
...
# write_xlsx(data1, "mat2b9c_28nov21_15h-16h.xlsx")

indx.int.point <- room.analysis$indx.in.point
  
plot(c(-80, 80), c(-80, 80), type="n", main =("In and out points") )
segments(Wall_lignes_firminy$`Start X'`, Wall_lignes_firminy$`Start Y'`, Wall_lignes_firminy$`End X'`, Wall_lignes_firminy$`End Y'`)
points(data1$x, data1$y, col=3, cex=0.1, pch=20)
points(data1$x[-indx.int.point ], data1$y[-indx.int.point ], col=2, cex=0.1, pch=20)

source("~/Devellyn/traitement_data position/Firminy installation/Analyse de trajectoires/Functions/traj_analysis.R")
traj <- traj_analysis(data1)
View(traj$traj)
